...
<body>
  <div class="container">
    <header>
      <a href="http://localhost:8080/board/list"><h1>롯데시네마</h1></a>
			...
			<section id="container">
				<form role="form" method="post" action="/board/write">
					<table class = "table table-hover">
						<tr><th>PostNumber</th><th>상품사진</th><th>상품설명</th><th>가격</th>Clothes<th>작성자</th><th>조회수</th><th>작성일</th><th>상태</th></tr>						
						<c:forEach items="${list_Clothes}" var = "list">
							<tr>
        ...
        <ul>
         ...
         <div class="page">
				    ...
				    <c:if test="${pageMaker.prev}">
				    	<li><a href="list_Clothes${pageMaker.makeQuery(pageMaker.startPage - 1)}"><<</a></li>
				    </c:if> 
				
				    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
				    	<li><a href="list_Clothes${pageMaker.makeQuery(idx)}">${idx}</a></li>
				    </c:forEach>
				
				    <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
				    	<li><a href="list_Clothes${pageMaker.makeQuery(pageMaker.endPage + 1)}">>></a></li>
				    </c:if> 
				  </ul>
				</div>
